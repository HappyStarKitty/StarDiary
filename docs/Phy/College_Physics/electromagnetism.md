...
$$\bm{P}=\varepsilon_0\chi_e\bm{E}=\varepsilon_0(\varepsilon_r-1)\bm{E}$$
...
!!! note 磁偶极子
    ...
    $e_n$为线圈平面正法线方向上的单位矢量，其方向与电流环绕的方向呈右螺旋关系
...
$$\bm{B}=\frac{dB}{dN}=\frac{\mu_0}{4\pi}\frac{q\bm{v}\times \bm{r}}{r^3}$$
...
$$
\oint_L \bm{B}\cdot d\bm{l}=\mu_0\sum I_{内}
$$
...
!!! note 解题步骤
    ...
    2. 根据$\bm{B}、\bm{M}、\bm{H}$的关系求出$\bm{B}、\bm{\mu_0I}$
...
$$\oint_s\bm{B}\cdot d\bm{S}=\oint_s(\bm{B}_0+\bm{B}^{\prime})\cdot d\bm{S}=0$$
...
Ⅰ式和Ⅲ式分别为电场高斯定理和磁场高斯定理的推广；Ⅱ式为法拉第电磁感应定律的推广；Ⅳ式为全电流安培环路定理

$$
\bm{B}=\varepsilon \bm{E}, \quad \bm{B}=\mu\bm{H}, \quad \bm{j}=\gamma \bm{E}
$$
...
$$
W=E_0H_0\cos^2\omega(t-\frac{r}{v})
$$

平均能流密度

$$\bar{S}=\frac{1}{2}E_0H_0$$